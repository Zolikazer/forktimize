...
class LinerProgrammingMenuCreatorAdvanced(MenuCreationAlgorithm):
    def run(self, foods: List[Food], nutrition_constraints: NutritionalConstraints) -> Menu:
        ...
        problem = self._add_calorie_constraints(foods, nutrition_constraints, problem, x_vars)

        # 3) Protein in [min_protein, max_protein] (if specified)
        problem = self._add_protein_constraints(foods, nutrition_constraints, problem, x_vars)

        # 4) Carbs in [min_carbs, max_carbs] (if specified)
        problem = self._add_carbs_constrains(foods, nutrition_constraints, problem, x_vars)

        # 5) Fat in [min_fat, max_fat] (if specified)
        problem = self._add_fat_constraints(foods, nutrition_constraints, problem, x_vars)

        # Add a universal max occurrences constraint if desired
        problem = self._add_max_occurance_per_food_constraint(foods, nutrition_constraints, problem, x_vars)
        ...
        menu = Menu(list())
        if status == "Optimal":
            chosen_foods = self._convert_result_to_menu(foods, menu, x_vars)
    ...
    def _add_max_occurance_per_food_constraint(self, foods, nutrition_constraints, problem, x_vars):
    ...
    def _convert_result_to_menu(self, foods, menu, x_vars):
        chosen_foods = {}
        for f in foods:
    ...
    def _add_fat_constraints(self, foods, nutrition_constraints, problem, x_vars):
    ...
    def _add_carbs_constrains(self, foods, nutrition_constraints, problem, x_vars):
    ...
    def _add_protein_constraints(self, foods, nutrition_constraints, problem, x_vars):
    ...
    def _add_calorie_constraints(self, foods, nutrition_constraints, problem, x_vars):
