...
def test_get_foods_for_given_date__returns_all_cityfood_on_matching_date(session):
    foods = get_foods_for_given_date(session, date(2025, 3, 18), FoodVendor.CITY_FOOD)
    assert len(foods) == 3
...
def test_get_foods_for_given_date__returns_empty_list_if_vendor_has_no_food(session):
    foods = get_foods_for_given_date(session, date(2025, 3, 20), FoodVendor.CITY_FOOD)
    assert foods == []
...
def test_get_foods_for_given_date__returns_interfood_correctly(session):
    foods = get_foods_for_given_date(session, date(2025, 3, 20), FoodVendor.INTER_FOOD)
    assert len(foods) == 1
